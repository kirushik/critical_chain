  ...
  class DashboardController < BaseController
    def index
      @stats = Rails.cache.fetch("admin_dashboard_stats", expires_in: 5.minutes) do
        seven_days_ago = 7.days.ago.to_s(:db)
        thirty_days_ago = 30.days.ago.to_s(:db)

        user_counts = User.connection.select_one(<<-SQL.squish)
          SELECT
            COUNT(*) AS total_users,
            COUNT(CASE WHEN created_at > '#{seven_days_ago}' THEN 1 END) AS users_last_7_days,
            COUNT(CASE WHEN created_at > '#{thirty_days_ago}' THEN 1 END) AS users_last_30_days,
            COUNT(CASE WHEN last_sign_in_at > '#{seven_days_ago}' THEN 1 END) AS active_users_last_7_days,
            COUNT(CASE WHEN last_sign_in_at > '#{thirty_days_ago}' THEN 1 END) AS active_users_last_30_days,
            COUNT(CASE WHEN banned_at IS NOT NULL THEN 1 END) AS banned_users
          FROM users
        SQL

        estimation_counts = Estimation.connection.select_one(<<-SQL.squish)
          SELECT
            COUNT(*) AS total_estimations,
            COUNT(CASE WHEN created_at > '#{seven_days_ago}' THEN 1 END) AS estimations_last_7_days,
            COUNT(CASE WHEN created_at > '#{thirty_days_ago}' THEN 1 END) AS estimations_last_30_days
          FROM estimations
        SQL
